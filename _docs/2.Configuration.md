...
return [
    ...
    'route'         => [
        ...
        'attributes' => [
            ...
            'middleware' => env('TOBYYAN_LOGVIEWER_MIDDLEWARE') ? explode(',', env('ARCANEDEV_LOGVIEWER_MIDDLEWARE')) : null,
        ],
    ],
...
By default no middleware is added to `log-viewer` route. If you need middlewares you just have to add a `TOBYYAN_LOGVIEWER_MIDDLEWARE` key to your `.env` file and add middlewares as comma separated values (no space).

 * **Example 1:** single middleware

```
TOBYYAN_LOGVIEWER_MIDDLEWARE=web
```

 * **Example 2:** multiple middlewares

```
TOBYYAN_LOGVIEWER_MIDDLEWARE=web,auth,custom-middleware
```
